...
type
  ...
  TNewLineEvent = procedure(Sender: TObject; NewLine: string;
    OutputType: TOutputType) of object;
  ...
  published
    ...
    property MaxTimeAfterBeginning: integer
      Read FMaxTimeAfterBeginning Write FMaxTimeAfterBeginning; //maximum time of execution
    property MaxTimeAfterLastOutput: integer
      Read FMaxTimeAfterLastOutput Write FMaxTimeAfterLastOutput;
 ...
 validates the wShowWindow member.}
  ...
  try
    repeat //main program loop
        ...
        begin
          case pBuf^[i] of
            ...
            begin
              ...
              begin
                Inc(i)
              end; //so we don't test the #10 on the next step of the loop
              FTimer.NewOutput; //a new ouput has been caught
              FLines.add(Str); //add the line
              if (FOutputLines <> nil) then
              begin
      ...
      else      //send lines in input (if exist)
      begin
    ...
    until ((Exit_Code <> STILL_ACTIVE) //process terminated (normally)
        or ((FMaxTimeAfterBeginning < FTimer.FSinceBeginning) and
        (FMaxTimeAfterBeginning > 0)) //time out
        or ((FMaxTimeAfterLastOutput < FTimer.FSinceLastOutput) and
        (FMaxTimeAfterLastOutput > 0))); //time out
    if (Last <> '') then
